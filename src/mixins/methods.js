	...
	methods: {
		...
		pickPage(check = true) {
			...
			if (check) {
				...
				this.$emit(
					'removed',
					this.picked.filter((val, index) => toDo.includes(index))
				)
		...
		remote(initPicked = false) {
			if (typeof this.data === 'string' && this.dataLoad && typeof this.dataLoad === 'function') {
				const queryParams =
					this.params && Object.keys(this.params).length ? JSON.parse(JSON.stringify(this.params)) : {}
				queryParams.pageSize = this.pageSize
				queryParams.pageNum = this.pageNumber
				...
				this.dataLoad(this, this.data, this.method, queryParams)
					.then(resp => {
						if (resp) {
							if (!this.resultFormat || typeof this.resultFormat !== 'function') {
								// eslint-disable-next-line no-console
								console.error(
									'In server side mode, you need specified "result-format" option(function type) to format server side response result.'
								)
							} else {
								const tmpObj = this.resultFormat(resp)
								if (tmpObj && Object.keys(tmpObj).length) {
									this.list = tmpObj.list
									this.totalRows = tmpObj.total
									if (!initPicked) {
										// load new page data list
									} else {
										// this.picked = tmpObj.list
										this.getSelected(tmpObj.list)
									} // the selected item info
								}
							}
						}
					})
		...
		getSelected(data) {
			...
			if (arr && arr.length) {
				const matchRows = data.filter(val => arr.includes(String(val[this.keyField])))
